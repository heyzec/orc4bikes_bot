...
def routes_button(update, context):
...
def payment_button(update, context):
...
def payment_pic(update, context):
...
def payment_done(update, context):
...
def rent_button(update, context):
...
def return_cancel(update, context):
    update.message.reply_text(
...
def report_desc(update, context):
...
def report_anything(update, context):
...
def report_done(update, context):
...
def report_cancel(update, context):
    update.message.reply_text(
...
conversation_handler = ConversationHandler(
    entry_points = [
        CommandHandler('rent', rent_command),
        CommandHandler('routes', routes_command),
        CommandHandler('payment', payment_command),
        CommandHandler('topup', payment_command),
    ...
    states = {
        # 1-X: Rental
        11: [
            CallbackQueryHandler(rent_button),
            CommandHandler('done', callback=rent_done),
        ],
        12: [
            CallbackQueryHandler(terms_button),
            CommandHandler('done', callback=rent_done),
        ],
        13: [
            MessageHandler(filters=~Filters.command, callback=rent_pic),
            CommandHandler('skip', rent_pic),
            CommandHandler('done', callback=rent_done),
        ],

        # 5-X: Routes
        51: [
            CallbackQueryHandler(routes_button),
            MessageHandler(filters=Filters.text | Filters.command, callback=lambda x, y:-1),
        ],

        # 7-X: Payment
        71: [
            CallbackQueryHandler(payment_button),
            CommandHandler('done', callback=payment_done),
        ],
        72: [
            CallbackQueryHandler(payment_pic),
            MessageHandler(filters=Filters.photo & ~Filters.command, callback=payment_pic),
            CommandHandler('skip', payment_pic),
            CommandHandler('done', callback=payment_done),
        ],

        # 8-X: Reports
        81: [
            MessageHandler(filters=Filters.text & ~Filters.command, callback=report_desc),
            CommandHandler('done', callback=report_done),
        ],
        82: [
            MessageHandler(filters=Filters.photo & ~Filters.command, callback=report_pic),
            CommandHandler('skip', report_pic),
            CommandHandler('done', callback=report_done),
        ],
        83: [
            MessageHandler(filters=(Filters.text | Filters.photo) & ~Filters.command, callback=report_anything),
            CommandHandler('done', callback=report_done),
        ],

        # 9-X: Returns
        91: [
            MessageHandler(~Filters.command, callback=return_pic),
            CommandHandler('skip', return_pic),
            CommandHandler('done', callback=return_done)
        ],
