...
class AdminBot(TeleBot):
    ...
    def orcabikes_command(self, update, context):
        if len(context.args) == 0:
            ...
            text = (
                f"<i>Bikes with {EMOJI['msg']} indicates a custom message is shown to user when they rent."
                " Use /orcabikes bikename to show the message.</i>"
                "\n\n"
            )
            text += '\n'.join(
                (
                    f'{bike["name"]} -- {bike["username"] or bike["status"] or EMOJI["tick"]}'
                    f' (Pin: {bike["pin"]}) {EMOJI["msg"] if bike["message"] else ""}'
                )
